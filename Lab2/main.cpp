...
void
computeFields(const std::vector<std::vector<ECE_PointCharge> > &matrix, int M, int startRow, int endRow, double &ex,
              double &ey, double &ez, double qx, double qy, double qz, double charge);
...
//    cout << "Your computer supports " << n << " concurrent threads." << endl;
    ...
    while(true) {
        ...
        double ex_total = 0.0, ey_total = 0.0, ez_total = 0.0;
#pragma omp parallel num_threads(threadNum) reduction(+:ex_total, ey_total, ez_total)
        {
            int tid = omp_get_thread_num();
            int startRow = tid * rowsPerThread;
            int endRow = (tid == threadNum - 1) ? N : startRow + rowsPerThread; // Last thread takes remaining rows

            double ex_thread = 0.0, ey_thread = 0.0, ez_thread = 0.0;

            computeFields(matrix, M, startRow, endRow, ex_thread, ey_thread, ez_thread, qx, qy, qz, charge);
        ...
        printInScientific(ex_total, "Ex");
        printInScientific(ey_total, "Ey");
        printInScientific(ez_total, "Ez");

        double eMagnitude = sqrt(ex_total * ex_total + ey_total * ey_total + ez_total * ez_total);
